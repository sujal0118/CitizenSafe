...
public class UpdateCase extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            if (rowsUpdated > 0) {
                ...
                // Send success message back to viewCase.jsp
                request.getSession().setAttribute("message", "✅ Case Updated Successfully.");
                request.getSession().setAttribute("messageType", "success");
            } else {
                request.getSession().setAttribute("message", "❌ Error Updating Case. Please try again.");
                request.getSession().setAttribute("messageType", "error");
            }
            
            response.sendRedirect("viewCase.jsp?caseId=" + caseId);
    ...
    private String generateCaseReport(int caseId) throws Exception {
        ...
        if (rs.next()) {
            ...
            while (evidenceRs.next()) {
                ...
                if ("recording".equalsIgnoreCase(evidenceType)) {
                    ...
                    try {
                        ...
                        document.add(new Paragraph(" Audio Transcription: " + transcript, textFont));
                    } catch (Exception e) {
            ...
            evidenceStmt.close();
            document.add(new Paragraph("\n--------------------------------------------\n", textFont));
            document.add(new Paragraph("⚠️ Important Notice ⚠️", headerFont));
            document.add(new Paragraph("This document is NOT a legally registered FIR.", textFont));
            document.add(new Paragraph("You must visit your nearest police station for further legal processing.", textFont));
            document.add(new Paragraph("\n--------------------------------------------\n", textFont));
